...
static SDImageOwner apply_upscaler(ServerRuntime& runtime,
    ...
    if (!upscale(ctx.get(), input, 4, &upscaled_images, &upscaled_count) ||
        upscaled_count <= 0 || upscaled_images[0].data == nullptr) {
        free_sd_images(upscaled_images, upscaled_count);
        LOG_ERROR("upscale() failed for: %s", upscaler_name.c_str());
        return SDImageOwner();
    }
    SDImageOwner upscaled(upscaled_images[0]);
...
bool perform_upscale(ServerRuntime& runtime,
    ...
    if (use_blend) {
        ...
        if (i1.width != i2.width || i1.height != i2.height || i1.channel != i2.channel) {
            // Outputs differ in size (e.g. upscaler_1 == "None" passes through at the
            // source resolution). Blending mismatched buffers would read out of bounds,
            // so skip it and keep upscaler_1's result.
            LOG_WARN("upscaler outputs differ in size (%ux%u vs %ux%u); skipping blend",
                     i1.width, i1.height, i2.width, i2.height);
        } else {
            const float v        = std::min(std::max(request.upscaler_2_visibility, 0.0f), 1.0f);
            const size_t npixels = static_cast<size_t>(i1.width) * i1.height * i1.channel;
